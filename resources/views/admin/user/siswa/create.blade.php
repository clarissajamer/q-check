...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-gray-100">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold text-gray-800">Tambah Siswa Baru</h2>
    ...
    <form action="{{ route('admin.users.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nis" value="{{ old('nis') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('nis') border-red-500 @enderror" placeholder="Nomor Induk Siswa">
            @error('nis') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
        </div>

        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2">Nama Lengkap</label>
            <input type="text" name="name" value="{{ old('name') }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('name') border-red-500 @enderror" placeholder="Nama Siswa">
        ...
        <div class="mb-6">
            ...
            <select name="eskuls[]" multiple class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 @error('eskuls') border-red-500 @enderror" style="height: 120px;">
                @foreach($eskuls as $eskul)
                    <option value="{{ $eskul->id }}" {{ in_array($eskul->id, old('eskuls', [])) ? 'selected' : '' }}>
                        {{ $eskul->nama_eskul }}
                    </option>
                @endforeach
            </select>
